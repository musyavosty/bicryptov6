...
| `backend/dist/src/api/(ext)/admin/ecosystem/blockchain/[id]/status.put.js` | 56–70 | `checkLicenseFileExists` always returns `true` | Backend requires a `.lic` file on disk before allowing blockchain enable — "License not activated" blocked all ecosystem blockchain activation |
| `backend/dist/src/api/admin/finance/exchange/provider/[id]/status.put.js` | 49–63 | `checkLicenseFileExists` always returns `true` | Same license file check — would block enabling any exchange provider that has a productId set |
